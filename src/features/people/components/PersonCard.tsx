...
export function PersonCard({ name, role, bio, imageSrc }: PersonCardProps) {
  return (
    <div className="bg-white p-8 md:p-12 rounded-lg shadow-sm border border-gray-100 flex flex-col h-full">
      <div className="relative self-center mb-10 mt-4">
        <div className="w-48 h-48 rounded-full overflow-hidden border-[6px] border-white shadow-lg z-10 relative bg-gray-200">
          {imageSrc && (
            <img src={imageSrc} alt={name} className="w-full h-full object-cover" />
          )}
        </div>
        {/* Red accent circle behind image */}
        <div className="absolute -top-4 -left-4 w-48 h-48 rounded-full border-2 border-primary -z-0"></div>
        {/* Floating icon */}
      ...
      <div className="flex-grow">
        <div className="text-primary font-bold text-sm mb-1">{role}</div>
        <h3 className="text-2xl font-heading font-black text-dark mb-4 uppercase">{name}</h3>
